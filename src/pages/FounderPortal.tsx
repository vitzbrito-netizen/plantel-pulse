...
import FounderEquipeTab from '@/components/FounderEquipeTab';
...
export default function FounderPortal() {
  ...
  const [activeTab, setActiveTab] = useState<'clientes' | 'equipe'>('clientes');
        ...
        {activeTab === 'clientes' ? (
          <>
            <div className="mb-8">
              ...
              <p className="text-sm text-muted-foreground">👇 Toque em um cliente para abrir o dashboard da equipe</p>
            </div>

            <div className="flex items-center gap-3 mb-6">
              <div className="flex items-center gap-2 bg-card border border-border rounded-lg px-4 py-2.5">
                <Building2 className="w-4 h-4 text-muted-foreground" />
                <span className="text-sm"><span className="font-bold text-foreground">{activeCount}</span><span className="text-muted-foreground"> ativo{activeCount !== 1 ? 's' : ''}</span></span>
              </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20 bg-card rounded-xl border border-border border-dashed">
                ...
                <button onClick={() => { setEditingClient(null); setFormOpen(true); }} className="text-sm text-primary font-semibold hover:underline">+ Criar primeiro cliente</button>
              </div>
            ) : (
          ...
          <FounderEquipeTab clients={clients} />
        )}
      </div>
    </div>
  );
